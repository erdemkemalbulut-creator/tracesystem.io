...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check if we already generated a question today
    ...
    if (existing) {
      return new Response(JSON.stringify({ question: existing.question_text }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (season) {
      ...
      if (trackables && trackables.length > 0) {
        // Get recent trackable completions
...
- Never use emojis in the question`;
    ...
    const userPrompt = context
      ...
      : `Generate a thoughtful first-day question for a new user of a behavioral tracking system. Something that helps them start noticing their patterns.`;
