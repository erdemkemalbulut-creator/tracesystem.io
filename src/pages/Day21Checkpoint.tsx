...
export default function Day21Checkpoint() {
  ...
  const [data, setData] = useState<Day21Data | null>(null);
  ...
  const handleContinue = async () => {
    ...
    try {
      await updateProfile({
        ...
        checkpoint_reflection_day_21: reflection
      });
      navigate('/today');
    } catch (err) {
  ...
  return (
    <div className="page-with-shell reflective-bg">
      ...
      <div className="page">
        ...
        <h1>Checkpoint — Day 21</h1>
        <p className="helper-text" style={{ marginBottom: '1rem' }}>
          Twenty-one days of entries are now recorded.
        </p>
        <p className="helper-text" style={{ marginBottom: '1rem' }}>
          At this point, behavior is no longer situational.
        </p>
        <p className="helper-text" style={{ marginBottom: '1rem' }}>
          What repeats under observation is what your actions default to.
        </p>
        <p className="helper-text" style={{ marginBottom: '2rem' }}>
          Trace does not label this.<br />
          It only shows what held.
        </p>
        ...
        <div className="milestone-section">
          <p className="section-label" style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-muted)', marginBottom: '1rem' }}>
            Evidence from the last 21 days
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '0.5rem' }}>
            <p style={{ fontSize: '0.95rem', lineHeight: '1.6', color: 'var(--color-text-tertiary)', margin: 0 }}>
              Alignment count: {data.alignmentCount} / 21
            </p>
            <p style={{ fontSize: '0.95rem', lineHeight: '1.6', color: 'var(--color-text-tertiary)', margin: 0 }}>
              Integrity count: {data.integrityCount} / 21
            </p>
          </div>
          <p className="helper-text" style={{ fontSize: '0.85rem' }}>
            This reflects what occurred, not what was planned.
          </p>
        </div>
        ...
        <div className="milestone-section">
          <label htmlFor="checkpoint-reflection" className="section-label" style={{ display: 'block', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-muted)', marginBottom: '0.5rem' }}>
            What does this pattern suggest about how you operate?
          </label>
          <textarea
          ...
          <p className="helper-text" style={{ marginTop: '0.5rem', fontSize: '0.875rem' }}>
            State what seems implied by repetition. Do not judge it.
          </p>
          {error && (
